...
for i = 0:t:time
    ...
    %%  Relation Equations
    rho_sup = Psup((uint64((i/t)+1)))*(R*Tsup);
    Aburn = 2*pi()*Rg*Lcomb;
    Vcomb = pi()*(Rg^2)*Lcomb+(Lpost+Lpre)*pi()*(Din/4)^2;
    Mprop = Mox + Mfuel;
    OF_ratio = Mox/Mfuel;
    ...
    dPcomb = ((Aburn*r)/Vcomb)*(rho_fuel*Rg*Tcomb-Pcomb)...
          ...
          +(Rg*Tcomb/Vcomb)*Aox*Cdox*sqrt(2*rho_sup*(Psup(uint64((i/t)+1))-Pcomb));
    dRg = r;
    Mox = Aox*Cdox*sqrt(2*rho_sup*(Psup(uint64((i/t)+1))-Pcomb));
...
plot(data(:,1), data(:,2),'-',data(:,1),data(:,9),'--',data(:,1), Psup(:),':','Linewidth',2)
